...
## 2025-05-23 - Keyboard Support for Custom Interactions
**Learning:** Custom components using `onMouseEnter`/`onMouseLeave` for visual effects (like scaling) often neglect keyboard users.
**Action:** Always mirror hover effects with `onFocus` and `onBlur` handlers to ensure keyboard users receive the same visual feedback.
