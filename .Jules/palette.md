...
## 2026-01-30 - Date Hydration Mismatches
**Learning:** `toLocaleDateString()` without fixed locale/timezone causes hydration errors in Docusaurus because server (UTC) and client (local) render different strings.
**Action:** Always use `.toLocaleDateString('en-US', { timeZone: 'UTC' })` for consistency.
