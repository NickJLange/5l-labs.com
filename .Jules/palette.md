...
## 2025-05-24 - Acronym Links and Keyboard Interactivity
**Learning:** Acronym-only links (like "OE") are opaque to screen readers and often lack keyboard interactivity when implemented with mouse-only handlers.
**Action:** Expand acronyms in `aria-label` and `title`, and ensure all hover effects have corresponding `onFocus`/`onBlur` handlers.
