...
## Code Review & Development Workflow

Review code thoroughly before making changes. For every issue or recommendation, explain the concrete tradeoffs, give an opinionated recommendation, and ask for user input before assuming a direction.
...
Apply the full review workflow when:
- Implementing an OpenSpec proposal (Stage 2 — before writing code)
- Reviewing a PR or set of changes
- Evaluating existing code for improvement
- Any change touching more than a handful of lines

Skip the formal review for:
...
Before starting a review, you **MUST** ask the user which mode they want:

1. **BIG CHANGE**: Work through all four review sections interactively, one section at a time (Architecture, Code Quality, Tests, Performance), with at most **4 top issues per section**
2. **SMALL CHANGE**: Work through all four sections interactively, with **1 top issue per section**

Use `AskUserQuestion` to present this choice before proceeding.

### Review Process

Work through each section in order. After presenting issues for a section, **pause and ask for feedback** before moving to the next section.
...
1. **NUMBER** each issue sequentially across the section (Issue #1, #2, #3, ...)
2. **Describe the problem concretely** with file and line references (`file.py:42`)
...
5. **Recommended option is always listed first (Option A)** — label it clearly (e.g., "1A (Recommended)")
6. **Use `AskUserQuestion`** at the end of each section so the user can select their preferred option per issue. Each `AskUserQuestion` option MUST clearly label the issue NUMBER and option LETTER so the user doesn't get confused

Example format:

```
**Issue #1**: `app/api/endpoints.py:45` — Embedding vector stored as JSON string instead of native pgvector type

  A (Recommended): Migrate to pgvector column type
    - Effort: Medium (migration + adapter change)
    - Risk: Low (pgvector is mature)
    - Impact: Enables native similarity search, removes JSON serialization overhead
    - Maintenance Burden: Lower long-term

  B: Add JSONB column as intermediate step
    - Effort: Low
    - Risk: Low
    - Impact: Better than string, but still no native vector ops
    - Maintenance Burden: Creates migration debt

  C: Do nothing
    - Effort: None
    - Risk: Performance degrades as data grows
    - Impact: None now, problematic at scale
    - Maintenance Burden: Accumulates tech debt
```
