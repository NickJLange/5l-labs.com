        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {items.map((item, idx) => (
            <li key={idx} style={{ marginBottom: '1rem' }}>
              <strong>
                {/* ⚡ Bolt: Use Docusaurus <Link> for internal navigation to enable SPA routing (faster than full page reloads) */}
                {item.link ? (
      ...
      <div className="text--center padding-horiz--md">
        ...
        <div className="card shadow--md">
          <div className="card__header">
            <h3>
              {/* ⚡ Bolt: Use <Link> for internal navigation */}
              <Link to={latestPost.url}>{latestPost.title}</Link>
            </h3>
            {/* ⚡ Bolt: Fixed locale and timezone to prevent server/client hydration mismatches */}
              ...
              {homepageConfig.products.map((product, idx) => (
                <div key={idx} className="col col--6 margin-bottom--md">
                  <div className="card shadow--md h-100">
                    <div className="card__header">
                      <h3>
                        {/* ⚡ Bolt: Conditional Link/a tag for performance */}
                        {product.link ? (
