...
function getLatestPost() {
    let newestFileMeta = null;
    const DATE_REGEX = /^(\d{4})-(\d{2})-(\d{2})/;

    // First pass: Find the newest file by filename date
    BLOG_DIRS.forEach(dir => {
        ...
        files.forEach(file => {
            ...
            if (!newestFileMeta || date > newestFileMeta.date) {
                newestFileMeta = { dir, dirPath, file, date, yearStr, monthStr, dayStr };
            }
        });
    });

    if (!newestFileMeta) return null;

    // Second pass: Read and parse only the newest file
    const { dir, dirPath, file, date, yearStr, monthStr, dayStr } = newestFileMeta;
